...
void set_thread_affinity(int tid, const affinity_mask_t& mask)
{
  ...
  buffer << std::this_thread::get_id() << " -> (" << tid << ", " << mask << ")\n";
  std::cout << buffer.str();
}
void restore_thread_affinity()
{
  std::ostringstream buffer;
  buffer << std::this_thread::get_id() << " -> (restored)\n";
  std::cout << buffer.str();
}
...
class PinningObserver : public tbb::task_scheduler_observer
{
 public:
  ...
  void on_scheduler_exit(bool worker) override { restore_thread_affinity(); }
};
...
void observeTwoArenas()
{
  ...
  a0.execute([] { tbb::parallel_for(0, N, [](int j) { f(w); }); });
  std::cout << "Execute a1 loop\n";
  a1.execute([] { tbb::parallel_for(0, N, [](int j) { f(w); }); });
